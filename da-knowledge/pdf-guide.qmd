---
title: "How to Generate a PDF Report"
subtitle: "Options for different setups"
---
...
**Requires:** Quarto + either R or Python + pandoc (bundled with Quarto)
...
```bash
python report.py
# The script generates .tex and calls xelatex automatically
```
...
**Requires:** Python + Jupyter + nbconvert (+ LaTeX for PDF)
...
# LaTeX needed for PDF export (see Option 3)
```
...
Also available from the Jupyter menu: **File > Export Notebook As > PDF**.

## Option 6: Python + fpdf2 (no LaTeX needed)

**Requires:** Python (pandas, matplotlib, fpdf2) only
...
|--------|-------------|-----------------|-------|
| **Quarto** | Quarto, R or Python | Yes (or use `typst`) | Best overall experience |
| **R Markdown** | R, rmarkdown | Yes (use `tinytex`) | Mature, many templates |
| **Python + LaTeX** | Python, TeX Live | Yes | Full layout control |
| **Python + pandoc** | Python, pandoc | Yes (HTML fallback: no) | Write in Markdown |
| **Jupyter export** | Python, Jupyter | Yes (HTML fallback: no) | Good for notebooks |
